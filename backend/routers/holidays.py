...
def add_holiday(payload: schemas.HolidayCreate, db: Session = Depends(get_db), _=Depends(require_role("admin"))):
    ...
    db.commit()
    db.refresh(h)
    return schemas.HolidayOut.model_validate(h)


@router.post("/today", response_model=schemas.HolidayOut, status_code=201)
def mark_today(db: Session = Depends(get_db), _=Depends(require_role("admin"))):
    today = date.today()
...
def delete_holiday(holiday_id: str, db: Session = Depends(get_db), _=Depends(require_role("admin"))):
